...
useEffect(() => {
  ...
  const getTransaction = (async () => {
    try{
      ...
      const response = await fetch('https://api.up.com.au/api/v1/accounts/ecd703f5-2bcd-4677-aed0-2f367a3e67a7/transactions', { 
        method: "GET",
        headers: {'Authorization': `Bearer ${import.meta.env.VITE_UP_API_KEY}`}
      })

      if (!response.ok) { //If the response is invalid then return an error with the status and message
        throw new Error(`Transaction Info Error: ${response.status} ${response.statusText}`);
      }
